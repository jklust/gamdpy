...
def FindMinimumEnthalpy(rho_array, ptype_unit_cell, plotindex = None):
...
rho_Cu, E_Cu, a_Cu = FindMinimumEnthalpy(rho_array, ptype_unit_cell=[0, 0, 0, 0], plotindex=1)
assert math.isclose(a_Cu, 3.615, rel_tol=0.001) # Gola et al 2018
if verbose:
    print(f"a_Cu and reference value {a_Cu} 3.615")
    
# Au
rho_array = np.arange(0.050, 0.070, 0.001)
rho_Au, E_Au, a_Au = FindMinimumEnthalpy(rho_array, ptype_unit_cell=[1, 1, 1, 1], plotindex=2)
assert math.isclose(a_Au, 4.080, rel_tol=0.001) # Gola et al 2018
if verbose:
    print(f"a_Au and reference value {a_Au} 4.080")


# Cu3Au
rho_array = np.arange(0.065, 0.085, 0.001)
rho_Cu3Au, E_Cu3Au, a_Cu3Au = FindMinimumEnthalpy(rho_array, ptype_unit_cell=[1, 0, 0, 0], plotindex=3)
...
if verbose:
    print(f"a_Cu3Au and reference value {a_Cu3Au} 3.750")
    print(f"E_mixing_Cu3Au and reference value {E_mixing_Cu3Au} -0.093")


# CuAu3
rho_array = np.arange(0.055, 0.075, 0.001)
rho_CuAu3, E_CuAu3, a_CuAu3 = FindMinimumEnthalpy(rho_array, ptype_unit_cell=[0, 1, 1, 1], plotindex=4)
...
if verbose:
    print(f"a_CuAu3 and reference value {a_CuAu3} 3.976")
    print(f"E_mixing_CuAu3 and reference value {E_mixing_CuAu3} -0.095")
