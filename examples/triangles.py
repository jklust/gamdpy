...
top = rp.Topology(['Triangle', ])
top.bonds = [[0, 1, 0], [0, 2, 0], [1, 2, 0], 
             [3, 0, 1], [3, 1, 1], [3, 2, 1]]
top.molecules['Triangle'] = rp.molecules_from_bonds(top.bonds)
...
rp.plot_molecule(top, positions, particle_types, filename="triangle.pdf", block=False)

configuration = rp.duplicate_molecule(top, positions, particle_types, masses, cells=(20, 20), safety_distance=4.0)
...
# Make integrator
integrator = rp.integrators.NVT_Langevin(temperature=temperature, alpha=0.4, dt=0.002, seed=23)
