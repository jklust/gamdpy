...
- [zbl.py](zbl.py) : Simulation of Cu using universal Ziegler–Biersack–Littmark (ZBL) potential.
...
- [lennard_jones_argon.py](lennard_jones_argon.py) : Lennard-Jones conversion from Argon units to real units
- [lennard_jones_argon_real_units.py](lennard_jones_argon_real_units.py) : Simulation of Lennard-Jones Argon using real units

## Molecular models

- [ASD.py](ASD.py) : ASymmetric Dumbbells (toy model of toluene).
- [LJchain.py](LJchain.py) : LJ chains (coarse grained polymer model).
- [LJchain_continue.py](LJchain_continue.py) : Continuation of LJ chains simulation.
...
- [water.py](water.py): Simulation of the SPC/Fw model 
- [water_real_units.py](water_real_units.py): Simulation of the SPC/Fw model using real units
...
- [active_ornstein_uhlenbeck.py](active_ornstein_uhlenbeck.py) : Purely repulsive active particles forming dense clusters (MIPS).
