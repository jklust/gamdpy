def test_Simbox():
    ...
    dr = 0*ri
    result = simbox.dist_sq_dr_function(ri, rj, simbox.lengths, dr)
    ...
    result = simbox.dist_sq_function(ri, rj, simbox.lengths)
    ...
    simbox.apply_PBC(r, img, simbox.lengths)
    ...
    ## Test volume
    assert simbox.volume(simbox.lengths)==3*4*5, "Problem with Simbox.volume"

    ## Test dist_moved_sq_function
    r_current = np.array([1,1,1])
    r_last    = r_current + 0.75*simbox.lengths
    result = simbox.dist_moved_sq_function(r_current, r_last, simbox.lengths, simbox.lengths)
    ...
    skin, cut = 2.e0, 1.e0
    result = simbox.dist_moved_exceeds_limit_function(r_current, r_last, simbox.lengths, simbox.lengths, skin, cut)
