...
class ConfSaver():
    ...
    def __init__(self, configuration, num_timeblocks:int, steps_per_timeblock:int, storage:str) -> None:
        ...
        self.conf_per_block = int(math.log2(self.steps_per_timeblock)) + 2 # Should be user controlable
        ...
        if self.storage[-3:]=='.h5': # Saving in hdf5 format
            with h5py.File(self.storage, "a") as f:
                ...
                ds = f.create_dataset("block", shape=(self.num_timeblocks, self.conf_per_block, self.num_vectors, self.configuration.N, self.configuration.D),
            ...
            self.output = {}
            self.output['block'] = np.zeros((self.num_timeblocks, self.conf_per_block, self.num_vectors, self.configuration.N, self.configuration.D), dtype=np.float32)
